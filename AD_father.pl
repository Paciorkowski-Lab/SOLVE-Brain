...
#Filters annotated vcf for paternally inherited Autosomal Dominant variants. User can supply the proband index. 
...
#filter for AD variants with paternal inheritance
LINE: while ($_=<$F>){
        if ($_ =~ m{Gene}){ print $_;}
	my @line = split /\t/;
